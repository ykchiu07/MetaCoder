    ...
    def generateModuleDetail(self, architecture_path: str, target_module_name: str, progress_data: Dict[str, Any], model_name: str,cancel_event: threading.Event = None ) -> ModuleDetailResult:
        ...
        **優化重點**：強化參數完整性 Prompt
        ...
        system_prompt = (
            ...
            f"1. This module depends on: {json.dumps(declared_deps)}.\n"
            "2. For EACH function you define, you MUST explicitly list which external APIs it calls from the dependencies.\n"
            "3. If a function connects two modules (e.g. `login` calls `auth_db.validate`), explicitly state it.\n"
            "\n"
            "Output strict JSON:\n"
            "{\n"
            "  'module_name': 'str',\n"
            ...
            "      'return_type': '...',\n"
            "      'docstring': '...',\n"
            "      'required_calls': ['mod_a.func_x', 'mod_b.func_y']\n"  # [關鍵新增]
            "    }\n"
            "  ]\n"
            "}"
        )
